...
test('desktop rail uses command search with structured filters in one panel', () => {
  ...
  assert.match(appSource, /City/);
  assert.match(appSource, /— Airport/);
  assert.match(appSource, /—— Lounge/);
  ...
  assert.match(appCss, /\.search-command-option\.is-depth-1 \.suggestion-copy\s*{/);
  assert.match(appCss, /\.search-command-option\.is-depth-2 \.suggestion-copy\s*{/);
