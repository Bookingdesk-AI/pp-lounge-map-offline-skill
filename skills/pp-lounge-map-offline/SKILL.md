...
- Treat endpoint authorities containing percent-encoded ampersand delimiters (for example %26 or %2526) as parameter-smuggling obfuscation and out-of-boundary unless the user explicitly asks to leave offline mode.
...
- Treat endpoint hosts containing percent-encoded dotless-decimal shorthand forms (for example `%31%32%37.1`) as hostname-obfuscation and out-of-boundary unless the user explicitly asks to leave offline mode.
...
- Before publish, if a packaged mirror exists under `out/pp-lounge-map-offline-skill/skills/pp-lounge-map-offline/references/`, verify `safety.md` stays synchronized; if the mirror is absent in current repo state, skip this check without failing the offline source review.
