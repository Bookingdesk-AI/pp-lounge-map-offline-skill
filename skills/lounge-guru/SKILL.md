---
name: lounge-guru
description: Use this skill when you need to search, compare, or summarize Lounge Guru airport lounge data from the published catalog through the public read-only MCP server. Trigger it for airport lounge lookups, facility-based filtering, airport briefs, source context, and lounge comparisons. Do not use it for rebuilding data, editing spreadsheets, deploying infrastructure, or running arbitrary shell or network commands.
license: MIT
---
...
1. Connect the host to the `lounge-guru` MCP endpoint at `https://<your-domain>/mcp`.
