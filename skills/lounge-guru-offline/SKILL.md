---
name: lounge-guru-offline
description: Use this skill when you need offline or air-gapped access to the bundled Lounge Guru airport lounge catalog. Trigger it for local lounge lookup, facility filtering, airport briefs, source context, and lounge comparisons when network access is unavailable or disallowed. Do not use it for data rebuilds, deploys, remote MCP endpoints, arbitrary shell execution, or any workflow that depends on live internet access.
metadata:
  {
    "openclaw":
      {
...
1. Start the local stdio MCP server with `node skills/lounge-guru-offline/scripts/run-offline-mcp.mjs`.
