...
const skillSourceDir = path.resolve(projectRoot, 'skills', 'pp-lounge-map');
const outDir = path.resolve(projectRoot, 'out', 'pp-lounge-map-skill');
...
async function main() {
  ...
  await fs.writeFile(
    ...
    `# pp-lounge-map skill\n\nThis repo is the public skill bundle mirror used for marketplace publishing.\n\n## Install\n\n- skills.sh: \`npx skills add <owner>/<repo>\`\n- ClawHub: publish the same bundle directory with semver.\n\n## Trust boundary\n\nThis bundle is read-only and is designed to work with the public \`pp-lounge-map\` MCP endpoint. It does not contain rebuild, deploy, spreadsheet, or secret-handling workflows.\n`,
    'utf8',
  );
